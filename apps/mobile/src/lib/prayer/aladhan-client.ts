import type {
  PrayerCalendarDayDto,
  PrayerCalendarPayload,
  PrayerName,
  PrayerTimingsPayload,
  PrayerTodayPayload,
} from "../../api/types";

import { formatCityCountryLabel } from "./location-label";
import { coerceCalculationMethod, CALCULATION_METHOD_OPTIONS } from "./prayer-methods";
...
export function splitTimingsIso(raw: Record<string, string>): {
...
function buildTimingsPayload(
  ...
  return {
    ...
    schoolLabel: school === 1 ? "Hanafi" : "Shafi (standard)",
    timings,
    timingsIso: Object.keys(timingsIso).length ? timingsIso : undefined,
  };
}
...
export async function fetchTodayByCity(opts: {
  ...
  const urlTom = new URL(`https://api.aladhan.com/v1/timingsByCity/${tomorrowDdMmYyyyUtc()}`);
...
export async function fetchTodayByCoords(opts: {
  ...
  const payload = buildTimingsPayload(
    day,
    "Approximate area (from your device)",
    method,
    opts.school,
  );
...
export async function fetchGregorianCalendarByCoords(opts: {
  ...
  const method = coerceCalculationMethod(opts.method);
  const url = new URL(`https://api.aladhan.com/v1/calendar/${opts.year}/${opts.month}`);
  const q = aladhanQueryBase(method, opts.school);
  q.set("latitude", String(opts.latitude));
  q.set("longitude", String(opts.longitude));
  if (opts.adjustment != null && Number.isFinite(opts.adjustment)) {
