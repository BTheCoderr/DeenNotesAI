/** Canonical Hafs mushaf approximate juz start points (surah:ayah → juz starts here). Read-only UX hint only (M5D prep). */
...
function atOrPast(surah: number, ayah: number, p: (typeof POINTS)[number]): boolean {
  return surah > p.surah || (surah === p.surah && ayah >= p.ayah);
}

/** Returns approximate jūz (1–30) for reading position using standard division starts. */
export function estimateJuz(surah: number, ayah: number): number {
  let j = 1;
  for (const p of POINTS) {
    if (atOrPast(surah, ayah, p)) j = p.juz;
  }
  return j;
}
