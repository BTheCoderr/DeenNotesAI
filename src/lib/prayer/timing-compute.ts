import type { PrayerName, PrayerScheduleDto } from "./types";

/** Ordered windows for obligatory salah context (sunrise omitted from “current” band). */
const SALAH_SEQUENCE: PrayerName[] = ["Fajr", "Dhuhr", "Asr", "Maghrib", "Isha"];

/** Local-instant milliseconds for today from ISO (preferred) or clock `HH:mm`. */
...
/**
 * Computes current / next salah using absolute instants when ISO strings exist.
 * Pass `tomorrowFajrIso` after Isha so Fajr shows the real next muezzin moment.
 */
